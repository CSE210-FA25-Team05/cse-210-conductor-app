...
module.exports = async function journalRoutes(fastify, options) {
    ...
    fastify.get(
        ...
        async (request, reply) => {
            ...
            const course_id = request.params.course_id;
            const res = await journalRepo.getJournalsByUserIdAndCourseId(user_id, course_id);
            return res;
            } catch (error) {
            console.error(error);
            return mapAndReply(error, reply);
    ...
    fastify.post(
        ...
        async (request, reply) => {
            ...
            const res = await journalRepo.createJournalEntry(student_id, course_id, title, content);
            return res;
            } catch (error) {
            console.error(error);
            return mapAndReply(error, reply);
    ...
    fastify.patch(
        ...
        async (request, reply) => {
            ...
            const res = await journalRepo.updateJournalEntry(journal_id, title, content);
            return res;
            } catch (error) {
            console.error(error);
            return mapAndReply(error, reply);
